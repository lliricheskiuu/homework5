...
result = []
tmp = int(len(my_str) / 2)

if not len(my_str) % 2:
    for index in range(tmp):
        result += [my_str[index * 2: index * 2 + 2]]
else:
    for index in range(tmp + 1):
        result += [my_str[index * 2: index * 2 + 2]]
    result[-1] += '_'
print(result)
...
print(my_str[my_str.find(l_limit) + 1:my_str.find(r_limit)])
